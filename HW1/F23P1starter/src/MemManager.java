...
/**
 * The semSanager is used to store the seminar objects, or retrieve seminar
 * objects from the memory pool.
 * 
 * @author Xianwei Wu/Jiren Wang
 * @version September 2023, updated September 2023
 * 
 * @param FreeList dummy: the dummy node, or it is called the header of the list
 * @param size: the size of the memory pool.
 * @param memory pool: the place to store seminar objects.
 * @param doubleSizeTest  the hash table size after rehashing
 */
...
public class MemManager {
	...
	/**
	 * The initializeSemManger is the method that is called to initialize the memory
	 * manager.
	 */
	...
	public void search(Handle handle) {
		/**
		 * The search function search the result by a given handle, and print the result
		 * out.
		 */
	...
	public FreeList FindSpaceAvailable(int requestedSize) {
		/**
		 * The function tries to find if there is a position available by a given
		 * requested size.
		 */
	...
	public int getNearestPowerOfTwo(int requestedSize) {
		/**
		 * The function return a power of two number that is greater than a given
		 * number.
		 */
...
//	        }
	...
	public Handle insert(byte[] insertData, int key) {
		/**
		 * The function inserts the data, when the memory pool deos not have enough
		 * space, it calls doubleSize().
		 * 
		 * @return once the record is stored, a handle is returned for recovery
		 *         purposes.
		 */
	...
	public FreeList detectMerge() {
		/**
		 * The function detects if any two nodes are adjacent and need to be merge to
		 * one node
		 */
		if (dummy == null) {
			return null;
